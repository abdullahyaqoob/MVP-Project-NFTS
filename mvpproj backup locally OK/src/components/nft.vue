    ...
    <div>
      ...
      <div class="secondDiv">
        <div class="table">
          ...
          />
          <br />
          <button v-if="!loading" class="checkBtn" @click="checkNFT()">Check It!</button>
          <button v-else class="checkBtn" @click="checkNFT()">Loading...</button>
          ...
          <tbody v-if="!loading">
            <tr v-for="(data, index) in requiredTraitsData" :key="index">
              ...
              <td data-title="ID" :title="data.nftUrl">
                {{ data.nftUrl.substring(0, 30) + " ..." }}<button
              class="tooltips"
            ...
            >
              ...
              <div class="traitsRow" style="text-align:start;margin-left:40px;margin-top:3px;" v-for="(traitss, index) in data.traits" :key="index">
                <span style="color: rgb(0, 10, 255)" :title="traitss.value">{{ traitss.value }}</span>
                <span style="margin-left: 20px; color: red;" :title="traitss.value">{{traitss.trait_count}}</span><br />
        ...
        <div v-if="loading">
          <br><br><br><br>
            <div style="box-shadow: 3px 0px 11px 4px rgb(200, 200, 200);width:40%;margin:auto;padding:7px"><div v-if="loading === true" class="lds-ring" style="height:28px"><div></div><div></div><div></div><div></div></div><span style="font-size: 20px;color:black;margin-left:20px">On Average It takes about 1 minute to fetch 1000 NFTs</span></div>
        </div>
      </div>
    </div>
  </div>
      ...
      try {
        ...
        this.requiredDataArray.forEach((data) => {
          data.traits.forEach((Sdata) => {
            var DataTraitCount = (Sdata.trait_count / this.allNftLength) * 100;
            if (DataTraitCount < 1) {
